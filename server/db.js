	...
	return new Promise((resolve, reject) => {
		...
		db.all(sql, [], (err, rows) => {
			if (err) {
				...
				let keywords = [];
				rows.forEach((elem) => keywords.push(elem.KEYWORD));
				resolve(keywords);
			}
		});
	});
...
exports.getUserInfo = (auth0) => {
	return new Promise((resolve, reject) => {
		const sql =
			'SELECT DISTINCT S.ID AS id_user, S.NAME AS name, S.SURNAME AS surname, S.EMAIL AS email FROM STUDENT S JOIN STUD_AUTH0 SA ON SA.ID == S.ID WHERE SA.ID_AUTH0 = ?';
	...
	return new Promise((resolve, reject) => {
		...
		db.all(sql, [], (err, rows) => {
			if (err) {
				...
				let types = [];
				rows.forEach((elem) => types.push(elem.TYPE));
				resolve(types);
			}
		});
	});
	...
	return new Promise((resolve, reject) => {
		...
		db.get(sql, [auth0.payload.sub], (err, elem) => {
			if (err) {
				...
				if (elem != undefined) resolve({ role: 'student', id: elem.ID });
				else {
					...
					db.get(sql2, [auth0.payload.sub], (err, elem) => {
						if (err) {
							...
							if (elem != undefined) resolve({ role: 'teacher', id: elem.ID });
							else resolve({}); //non ritorna nulla se non è autenticato, ma qua non entra
						}
					});
				}
			}
...
exports.getThesisTeacher = (ID) => {
	return new Promise((resolve, reject) => {
		const sql =
			'SELECT DISTINCT T.ID_THESIS as ID, T.TITLE AS title, T.NOTES as notes, T.DESCRIPTION AS description , T.REQUIRED_KNOWLEDGE AS req_know, TE.NAME AS sup_name, TE.SURNAME AS sup_surname FROM THESIS T JOIN TEACHER TE ON T.SUPERVISOR == TE.ID WHERE TE.ID = ?';
...
exports.getThesisStudent = (ID) => {
	return new Promise((resolve, reject) => {
		const sql =
			'SELECT DISTINCT T.ID_THESIS as ID, T.TITLE AS title, T.NOTES as notes, T.DESCRIPTION AS description , T.REQUIRED_KNOWLEDGE AS req_know, TE.NAME AS sup_name, TE.SURNAME AS sup_surname FROM THESIS T JOIN STUDENT S ON S.COD_DEGREE == T.DEGREE JOIN TEACHER TE ON T.SUPERVISOR == TE.ID WHERE S.ID = ?';
		db.all(sql, [ID], (err, rows) => {
			if (err) {
				reject(err);
				return;
...
exports.insertThesis = (title, description, req_know, notes, exp_date, level, degree, supervisor) => {
	return new Promise((resolve, reject) => {
		const sql =
			'INSERT INTO thesis (title, description, required_knowledge, notes, expiration_date, level, degree, supervisor) VALUES(?, ?, ?, ?, ?, ?, ?, ?)';
