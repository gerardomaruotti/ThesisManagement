...
const db = new sqlite.Database('thesis_management.db', (err) => {
  ...
  exports.getThesisTeacher=(ID)=>{
    return new Promise((resolve, reject) => {
      const sql = 'SELECT T.ID_THESIS as ID, T.TITLE AS title, T.NOTES as notes, T.DESCRIPTION AS description , T.REQUIRED_KNOWLEDGE AS req_know, TE.NAME AS sup_name, TE.SURNAME AS sup_surname FROM THESIS T JOIN TEACHER TE ON T.SUPERVISOR == TE.ID WHERE TE.ID = ?';
  ...
  exports.getThesisStudent=(ID)=>{
    return new Promise((resolve, reject) => {
      const sql = 'SELECT T.ID_THESIS as ID, T.TITLE AS title, T.NOTES as notes, T.DESCRIPTION AS description , T.REQUIRED_KNOWLEDGE AS req_know, TE.NAME AS sup_name, TE.SURNAME AS sup_surname FROM THESIS T JOIN STUDENT S ON S.COD_DEGREE == T.DEGREE JOIN TEACHER TE ON T.SUPERVISOR == TE.ID WHERE S.ID = ?';
      db.all(sql, [ID], (err, rows) => {
        if (err) { 
            reject(err); 
            return;
