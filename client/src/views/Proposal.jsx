...
function Proposal(props) {
	...
	function apply() {
		API.ThesisApply(id, props.accessToken)
			...
			.catch(() => {
				setMsgAndColor({ header: "Application failed", msg: "You have already sent an application for this thesis or you do not have authorization", color: "danger" });
				setPopup(true)
			});
	}
							...
							<Card style={{ padding: 20, cursor: 'pointer' }} className='custom-card'>
								<Row>
									<Col className='d-flex align-items-center'>
										<Button variant='outline-primary' style={{ borderRadius: 50, width: 75 }} onClick={() => { props.isProfessor ? navigate('/professor') : navigate('/') }}>
